...
//    gps_update_info(gps, gps->info);
    ...
    wait_time = 10;
    init_try_time = 0;
    LOGD(TAG" init_flag = %dl,param->test_type=%d\n", init_flag,param->test_type);
	}
    if(param->test_type == FTM_MANUAL_ITEM)
    {
    ...
    do {
        ...
        LOGD(TAG "%s, chosen = %d,ITEM_HTTFF=%d,ITEM_CTTFF=%d,ITEM_PASS,TEM_FAIL=%d\n", __FUNCTION__, chosen,ITEM_HTTFF,ITEM_CTTFF,ITEM_PASS,ITEM_FAIL);
        switch (chosen) {
        case ITEM_HTTFF:
            httff = 1;
            break;
