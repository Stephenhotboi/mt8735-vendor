...
#define WAVE_PLAY_MAX_TIME  (20000)   //in ms.
...
static int high_value =0;
static int pre_high_value =0;
...
static unsigned int g_MaxAmplitude = 0;
...
//wisky-lxh@20130826
unsigned int  trackMaxAmplitude(unsigned int *data, int times) {
	unsigned int tem = 0;
...
static void *Audio_Record_thread(void *mPtr)
{
    ...
    while (1)
    {
		...
		g_MaxAmplitude =magDataL[0]/32768;
	ALOGD("--->g_MaxAmplitude:%d,hds->i4OutputType=%d",g_MaxAmplitude,hds->i4OutputType);
...
int mAudio_receiver_entry(struct ftm_param *param, void *priv)
{
    ...
    if(FTM_AUTO_ITEM == param->test_type)
    {
        ...
        if (1)//(outputType == NULL) //wisky-lxh@20150316
        {
...
static void Audio_mic_info(struct mAudio *hds, char *info)
{
    int fd = -1;
    int hookstatus = 0;
    char *ptr ;
 ALOGD("audio_mic_info--->\n");
    //zla@20131113 
   if(g_MaxAmplitude > high_value)
		 high_value = g_MaxAmplitude ;
   if(high_value == pre_high_value){
	repeat_count ++;
   }
   if(repeat_count > 20) {
	   	flag_value = 0;
   ...
   if(high_value >= 10) {
	   ptr = info;
	   ptr += sprintf(ptr, "%s:%d \n\n",uistr_info_mic_note,high_value);
	   ALOGD("audio_mic_info--->1\n");
   	}
   else {
   		ptr = info;
	   ptr += sprintf(ptr, "%s:%d \n\n",uistr_info_mic_note,0);
	 ALOGD("audio_mic_info--->2\n");  
   	}
...
EXIT:
   ...
   if(high_value >= 10) {
	   ptr = info;
	   ptr += sprintf(ptr, "进行这项测试前请插入耳机:%d \n\n",high_value);
   	}
   else {
   		ptr = info;
	   ptr += sprintf(ptr, "进行这项测试前请插入耳机:%d \n\n",0);
...
static void *Audio_Mic_Loopback(void *mPtr)
{
    struct mAudio *hds = (struct mAudio *)mPtr;
    struct itemview *iv = hds->iv;
    ...
    while (1)
    {
        ...
        Audio_mic_info(hds, hds->info);  // get headset information
        iv->set_text(iv, &hds->text);
        iv->redraw(iv);
    }
...
int Audio_mic_loopback_entry(struct ftm_param *param, void *priv)
{
...
//        sprintf(mc->info, "%s", "进行这项测试前请插入耳机!\n"); 
//        iv->redraw(iv);
//        usleep(3000*1000);
//        return -1;
    }
    ...
    g_mic_change = 0;
   	...
   	Audio_mic_info(mc, mc->info); 

   pthread_create(&mc->hHeadsetThread, NULL, Audio_Mic_Loopback, priv);
   pthread_create(&mc->hHeadsetThread, NULL, Audio_Record_thread, priv);
...
//wisky-lxh@20130826
	...
	ret = ftm_register(modPMic_Headset_Loopback, Audio_mic_loopback_entry, (void*)maudioPMic_Headset_Loopback);
//end-wisky-lxh	
